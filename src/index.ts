...
function buildServer(client: ReturnType<typeof createYouTubeClient>) {
...
// ── Startup: stdio (local/Claude Code) or HTTP (remote/Railway) ────────
...
async function main() {
  try {
    ...
    if (port) {
      // Remote mode: Streamable HTTP transport for hosted deployments
      // (Railway, etc.) so it can be added to Claude.ai as a custom connector.
      ...
      // Simple health check for Railway / manual checks
      app.get('/', (_req, res) => {
        res.status(200).send('YouTube MCP Server is running');
      });

      app.post('/mcp', async (req, res) => {
        try {
          // Stateless: a fresh server + transport per request avoids
      ...
      // GET/DELETE on /mcp are used for SSE streams / session teardown in
      // the stateful variant of the protocol; not needed in stateless mode.
      app.get('/mcp', (_req, res) => res.status(405).end());
      app.delete('/mcp', (_req, res) => res.status(405).end());

      app.listen(Number(port), '0.0.0.0', () => {
        console.error(`YouTube MCP Server (HTTP) listening on port ${port}`);
      });
    } else {
